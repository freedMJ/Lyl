...
#页面加载数据接口
class LoadView(LoginRequiredMini,View):
	def get(self,request,pindex):
		orderMethod=0
		a=0
		key_str='result_time_%s_%d'%(orderMethod,a)
		...
		if flage:
			...
			try:
				olist=Orders.objects.filter(created_at__gt='%s'%start_time_stamp,created_at__lt='%s'%end_time_stamp)
			except Exception as e:
				HttpResponse("查询失败")

			try:
				print("aaa")
				key='result_load_%s_%d'%(orderMethod,a)
				print(key)
...
class ResultView(LoginRequiredMini,View):
	def get(self,request,pindex):
		...
		#key_str=request.session.get('key')
		...
		if flage:	
			...
			try:
				...
				#key='result_olist_%s_%s_%s_%s_%s_%s'%(start_time_stamp,end_time_stamp,usertoken,orderno,orderType,isfilled)
				print("aaa")
		...
		page_list=get_page.get_pages(int(total_page_number),int(pindex))
		print("333")
		return render(request,'htmls/template.html',{'page':page,'page_list':page_list,'olength':olength,'len_pindex':len_pindex,'pindex':pindex})
...
#跳转首页
def index(request):
	return render(request,'htmls/test2.html')
